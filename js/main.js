document.addEventListener('DOMContentLoaded', () => {
  ...
  const runJustifiedGallery = (item, data, isButton = false, tabs) => {
    ...
    const getItems = (nextGroupKey, count) => {
      ...
      // ===
      const startCount = nextGroupKey === 1 ? 0 : firstLoad + (nextGroupKey - 2) * increment;
      // <<<
    ...
    const addButton = item => {
      ...
      button.addEventListener('click', e => {
        ...
        // ===
        appendItem(ig.getGroups().length + 1, increment)
        // <<<
      }, { once: true })
    ...
    // ===
    const maxGroupKey = 1 + Math.ceil((dataLength - firstLoad) / increment)
    // <<<
    ...
    const requestAppendFn = btf.debounce(e => {
      ...
      // ===
      appendItem(nextGroupKey, increment)
      // <<<
